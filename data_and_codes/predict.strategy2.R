...
for (cyear in 2011:2012) {
  for (cmth in 1:12) {
    ...
    prefix <- "saved_models_accumulate/fit."
...
errors1 <- data.frame(in.sample.error = c(insample.rf.error.vec,insample.earth.error.vec),
                     out.sample.error = c(outsample.rf.error.vec,outsample.earth.error.vec),
                     type = c(rf.id,earth.id))

write.csv(errors1, file = "errors_accumulate.csv")
